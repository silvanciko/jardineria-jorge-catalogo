/**
 * dashboard.js — Panel privado (Fase 3B)
 * ------------------------------------------------------------------
 * Cada formulario edita una parte del catálogo en memoria y luego
 * guarda el objeto COMPLETO en /api/save (así lo exige el backend:
 * ver REQUIRED_KEYS en api/save.js). Esto evita guardados parciales
 * que dejen el catálogo en un estado a medio construir.
 *
 * Si en cualquier momento el servidor responde 401 (sesión vencida),
 * mandamos de vuelta al login — nunca dejamos el panel "medio abierto".
 * ------------------------------------------------------------------
 */
...
(async function guardSession(){
  try{
    ...
    setNotice("No hay conexión con el servidor. ¿Este proyecto ya está desplegado en Vercel?", true);
  }
})();
...
/** Si el servidor dice que la sesión ya no es válida, corta todo y manda al login. */
...
async function loadCatalog(){
  try{
    ...
    setNotice("No se pudo cargar el catálogo desde el servidor (¿ya desplegaste en Vercel con KV configurado?).", true);
  }
}
...
/** Guarda el catálogo completo (con lo que se acaba de modificar) en el backend. */
...
document.getElementById("siteTextForm").addEventListener("submit", async (e) => {
  ...
  try{
    ...
    setStatus("siteTextStatus", "No se pudo guardar. Intenta de nuevo.", true);
  }
});

/* ---------- Formulario: Footer (contacto y redes) ---------- */
document.getElementById("footerForm").addEventListener("submit", async (e) => {
  ...
  try{
    ...
    setStatus("footerStatus", "No se pudo guardar. Intenta de nuevo.", true);
  }
});

/* ---------- Compresión de imágenes: WebP con respaldo a JPEG ---------- */
/** Algunos navegadores viejos no saben codificar WebP en <canvas> y devuelven
 *  silenciosamente un PNG (pesado) en su lugar. Detectamos eso una sola vez. */
...
/* =========================================================
   Constantes de formulario (copia liviana de productos.js;
   se mantienen separadas a propósito: admin y sitio público
...
const DIFFICULTY_MAP = {
  facil: "🟢 Fácil", intermedio: "🟡 Intermedio", experto: "🔴 Experto",
};
const INFO_FIELDS = [
  ...
  wrap.innerHTML = catalogData.categories.map(c => `
    <div class="admin-mini-card">
      ...
      <div class="mini-actions">
        <button class="btn-save" data-edit-cat="${c.id}">Editar</button>
      </div>
    </div>
  `).join("");
  wrap.querySelectorAll("[data-edit-cat]").forEach(b =>
    b.addEventListener("click", () => openCategoryModal(b.dataset.editCat)));
}

function openCategoryModal(catId){
  const isNew = !catId;
  const cat = isNew ? { id: null, emoji: "🌿", name: "", eyebrow: "", desc: "" }
  ...
  if(!isNew){
    document.getElementById("mDel").addEventListener("click", async () => {
      const msg = count > 0
        ? `Esta sección tiene ${count} producto(s) que se borrarán también. ¿Continuar?`
        : `¿Eliminar "${cat.name}"?`;
...
function renderProductsAdmin(){
        ...
        <div class="admin-grid">
          ${products.map(p => `
            <div class="admin-mini-card">
              ...
              <p>${esc(p.price)}</p>
              <div class="mini-actions">
                <button class="btn-save" data-edit-prod="${cat.id}|${p.id}">Editar</button>
              </div>
            </div>
          `).join("")}
  ...
  wrap.querySelectorAll("[data-add-prod]").forEach(b =>
    b.addEventListener("click", () => openProductModal(b.dataset.addProd, null)));
}
...
function openProductModal(catId, prodId){
  ...
  document.getElementById("mSave").addEventListener("click", async () => {
    const finish = async (imgUrl) => {
      ...
      if(isNew){
        Object.assign(prod, data, { id: "p" + Date.now(), img: imgUrl || "", comments: [] });
    ...
    uploadImageIfSelected("mImg", finish);
  });
...
function renderServicesAdmin(){
  ...
  wrap.innerHTML = catalogData.services.map(s => `
    <div class="admin-mini-card">
      ...
      <h4>${esc(s.name)}</h4>
      <p>${esc(s.desc)}</p>
      <div class="mini-actions"><button class="btn-save" data-edit-svc="${s.id}">Editar</button></div>
    </div>
  `).join("");
  wrap.querySelectorAll("[data-edit-svc]").forEach(b =>
    b.addEventListener("click", () => openServiceModal(b.dataset.editSvc)));
}

function openServiceModal(svcId){
  const isNew = !svcId;
  const svc = isNew ? { id: null, iconKey: "poda_cesped", name: "", desc: "", img: "" }
                     : catalogData.services.find(s => s.id === svcId);
  ...
  document.getElementById("mSave").addEventListener("click", async () => {
    const finish = async (imgUrl) => {
      ...
      if(isNew){
        Object.assign(svc, data, { id: "s" + Date.now(), img: imgUrl || "" });
...
/**
 * Si el input de archivo tiene una foto seleccionada, la comprime y sube
 * a /api/upload; si no, llama a onDone(null) directo. Evita repetir este
 * bloque en cada modal que maneja una imagen (producto, servicio, logo).
 */
async function uploadImageIfSelected(inputId, onDone){
  ...
  try{
    ...
    if(await handleUnauthorized(up)) return;
